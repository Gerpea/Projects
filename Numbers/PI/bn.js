...
function BNsetdps(nbdps) {
  if (BNdps() != nbdps + 5) {
    BigNumber.config({
      //EXPONENTIAL_AT: ndps,  //Keep all output to max nb digits
...
function fw(n, fwd) {
  ...
  if (!isInt(fwd) || fwd <= 0)
    HALT('Function fw needs a positive whole number for the second argument')
...
function BNegcd(x, y) {
  ...
  while (!b2.eq(0)) {
    q = a2.divToInt(b2)
    z = a0.minus(q.multipliedBy(b0))
    a0 = b0
    b0 = z
    z = a1.minus(q.multipliedBy(b1))
    a1 = b1
    b1 = z
    z = a2.minus(q.multipliedBy(b2))
    a2 = b2
    b2 = z
...
function BNlcm(a, b) {
  if (typeof a == 'number') a = new BigNumber(a)
  if (typeof b == 'number') b = new BigNumber(b)
  return a.multipliedBy(b).div(BNgcd(a, b))
}
...
function BNsqrfactor(n) {
  ...
  //if(n<9)return f;
  for (
    var i = new BigNumber(3), ii = new BigNumber(9);
    n.gte(ii) && i.lte(lim);
    i = i.plus(2), ii = i.multipliedBy(i)
  ) {
...
function BNrandomInt(lo, hi, width) {
  //all ints in range lo..hi inclusive are equally likely
  ...
  do {
    var r = BigNumber.random(width).multipliedBy(hi.minus(lo).plus(1)).plus(lo).floor()
...
function BNmax(BNrow) {
  ...
  for (var i = 1, maxi = 0; i < row.length; i++) if (row[i].gt(row[maxi])) maxi = i
  // putmsg("BNmax of "+BNrow+" is@"+maxi+" = "+BNrow[maxi]);
  return row[maxi]
}

function BNmaxabs(BNrow) {
  //return the element with max ABS value, may be -ve
  if (arguments.length != 1 || !isArray(BNrow)) HALT('?? BNmax expects an array of BN')
  var row = arguments.length == 1 ? BNrow : arguments
  if (row.length == 0) HALT('?? BNmax given empty Array')
  for (var i = 1, maxi = 0; i < row.length; i++) if (row[i].abs().gt(row[maxi].abs())) maxi = i
  // putmsg("BNmax of "+BNrow+" is@"+maxi+" = "+BNrow[maxi]);
  return row[maxi]
}
...
function BNlog(x) {
  ...
  var xIN = x,
    ...
    pow = 0,
    z
  if (x.isNegative()) HALT('!! Cannot take the log of a negative number: ' + x)
  else if (x.isZero()) L = BN1
  else {
    if (x.gt(e)) {
      pow = -1
      epow = BN1
      while ((z = epow.multipliedBy(e)).lt(x)) {
        epow = z
        pow++
      }
      x = x.div(epow)
    ...
    if (x.lt(BN1)) {
      pow = 0
      epow = e
      while (x.multipliedBy(epow).lt(1)) {
        pow--
        epow = epow.multipliedBy(e)
      }
      x = x.multipliedBy(epow)
      pow--
    }
    ...
    while (term.gt(zero)) {
      i += 2
      term = term.multipliedBy(ysqr)
      L = L.plus(term.div(i))
    }
    L = L.multipliedBy(2).plus(pow)
    // putmsg("BNlog max term was "+i+" lib val="+Math.log(xIN));
  }
  return L
}

BigNumber.prototype.powmod = function (Pow, Mod) {
  //works on larger Powers than built-in BN.pow
  //pow::int|BN, mod::BN
  ...
  // else if(Pow.eq(1))pm= this.mod(Mod)
    ...
    pm = pmod2.isZero() ? ss : this.multipliedBy(ss).mod(Mod)
  }
...
function BNEpow(x) {
  ...
  while (term.abs().gt(zero) && i < 500) {
    i++
    term = term.multipliedBy(x).div(i)
    ans = ans.plus(term)
  }
...
function BNpow(b, p) {
  //BigNumber.pow only accepts integer powers!
  if (typeof b == 'number' || typeof b == 'string') b = new BigNumber(b)
  ...
  else ans = BNEpow(BNlog(b).multipliedBy(p))
...
BigNumber.prototype.degtorad = function () {
  return this.div(45).multipliedBy(BN1.arctan())
}
BigNumber.prototype.radtodeg = function () {
  return this.div(BN1.arctan()).multipliedBy(45)
}
BigNumber.prototype.sin = function () {
  var x = new BigNumber(this),
    sign = 1,
    piover4 = BN1.arctan()
  var twopi = piover4.multipliedBy(8),
    halfpi = piover4.multipliedBy(2),
    pi = piover4.multipliedBy(4)
  if (x.isNegative()) {
    sign = -1
    x = x.neg()
  } // 0<=x since sin(-x)=-sin(x)
  x = x.minus(x.div(twopi).floor().multipliedBy(twopi)) // 0<=x<=TwoPi
  if (x.gte(pi)) {
    x = x.minus(pi)
    sign *= -1
...
BigNumber.prototype.cos = function () {
  var x = new BigNumber(this),
    sign = 1,
    piover4 = BN1.arctan()
  var halfpi = piover4.multipliedBy(2),
    twopi = piover4.multipliedBy(8),
    pi = piover4.multipliedBy(4)
  if (x.isNegative()) {
    x = x.neg()
  ...
  return sign < 0 ? halfpi.minus(x).sin().neg() : halfpi.minus(x).sin() //since cos(x)=sin(Pi/2-x)
}
BigNumber.prototype.cosec = function () {
  return BN1.div(this.sin())
}
BigNumber.prototype.tan = function () {
  // up to about 250 dps
  BNsetdps(BNdps() + 9)
  var pi = BN1.arctan().multipliedBy(4)
  var a = this.mod(pi),
    sgn
  if (a.isNegative()) {
    a = a.neg()
...
BigNumber.prototype.arctan = function () {
  ...
  var delta = new BigNumber('1.0e-' + (BNdps() + 2).toString()),
    sgn,
    x = this
  if (x.isNegative()) {
    sgn = -1
  ...
  if (x.gt(1)) {
    s = BN1.arctan().multipliedBy(2).minus(BN1.div(x).arctan())
  }
  // else if(x.gt(0.5))s=this.div(this.sqr().plus(1).sqrt().plus(1)).arctan().multipliedBy(2)
  else {
    var y = x.sqr().div(x.sqr().plus(1)),
      s = x.div(x.sqr().plus(1)),
      k = 1,
      lastt = BN0
    var t = s
...
BigNumber.prototype.arccos = function () {
  ...
  return ans.isNegative() ? ans.plus(BN1.arctan().multipliedBy(4)) : ans
}

BigNumber.prototype.sinh = function () {
  return this.exp().minus(this.neg().exp()).div(2)
}
BigNumber.prototype.cosh = function () {
  //result is >=1
  return this.exp().plus(this.neg().exp()).div(2)
}
BigNumber.prototype.tanh = function () {
  //returns a value in range -1..1
  var e2 = this.multipliedBy(2).exp()
...
BigNumber.prototype.arccosh = function () {
  // 'this' must be >=1
  return this.lt(1)
    ? HALT('arccosh was given an argument <1:' + this)
    : this.sqr().minus(1).sqrt().plus(this).log()
}
BigNumber.prototype.arctanh = function () {
  //'this' must be in range -1..1
  ...
  BNsetdps(BNdps() + 5)
  var ans = this.plus(1).div(this.neg().plus(1)).log().div(2)
  BNsetdps(BNdps() - 5)
  return ans
}
...
BigNumber.prototype.factorial = function () {
  ...
  var f = BN1
  for (var i = 1; this.ge(i); i++) f = f.multipliedBy(i)
  return f
}

BigNumber.prototype.factorialNbDigits = function (Base) {
  if (arguments.length == 0) Base = BN10
  return BNpi.multipliedBy(2)
    .multipliedBy(this)
    .log(Base)
    .div(2)
    .plus(this.div(BNe).log(Base).multipliedBy(this))
    .plus(1)
    .floor()
}
...
BigNumber.prototype.factorialApprox = function () {
  ...
  for (var i = 1, f = BN1; this.ge(i); i++) f = f.multipliedBytipliedBy(i)
  BigNumber.config({ DECIMAL_PLACES: dps })
  return f
}
//return this.multipliedBy(2).plus(BN1.div(3)).multipliedBy(BNpi).sqrt().multipliedBy(this.topow(this)).multipliedBy(BNEpow(this.neg()))};

function BNbincoeff(r, c) {
  //n and r are int, return BN
  var ans = BN1,
  ...
  if (rr.isNegative() || cc.isNegative() || cc.gt(rr)) ans = BN0
    ...
    //if(r-c<c)c=r-c;
    //for(var i=r;i>=r-c+1;i--)ans=ans.multipliedBy(i);
    ans = rr.factorial().div(rr.minus(cc).factorial().multipliedBy(cc.factorial()))
  }
...
function bntest(dps) {
  ...
  var tests = [
    [
      'BN1.exp()',
      '2.',
    ],
    [
      'BNpi',
      '3.',
    ],
    ['(new BigNumber(90)).degtorad().minus(BNpi.div(2))', '0'],
    ['BNpi.div(2).radtodeg().minus(90)', '0'],
    ...
    ['BNpi.minus(BN1.arctan().multipliedBy(4))', '0'],
    ["(new BigNumber('0.3')).arcsinh().sinh()", '0.3'],
    ["(new BigNumber('0.3')).cosh().arccosh()", '0.3'],
    ["(new BigNumber('0.3')).arctanh().tanh()", '0.3'],
    ['BN1.plus(BNphi).minus(BNphi.sqr())', '0'],
    ['BN1.div(BNphi).sqr().plus(BNphi)', '2'],
    ['BN2.exp().log()', '2'],
    ['BN1.sinh().minus(BN1.exp().div(2)).plus(BN1.div(BN1.exp().multipliedBy(2)))', '0'], //sinh(1)-e/2+1/(2*e)","0"],
    ['BN1.cosh().minus(BN1.exp().div(2)).minus(BN1.div(BN1.exp().multipliedBy(2)))', '0'], //"cosh(1)-e/2-1/(2*e)","0"],
    ['BN1.tanh().minus(BN1.exp().sqr().minus(1).div(BN1.exp().sqr().inc()))', '0'], //"tanh(1)-(e^2-1)/(e^2+1)","0"]
  ]
  var ok = true,
    a
...
BigNumber.prototype.toBaseDigits = function (b) {
  ...
  var ds = new Array(),
    BigNumberBaseChrs = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ$_'
  if (b instanceof BigNumber) {
    b = b.toNumber()
    if (!isInt(b)) HALT('BigNumber tobase: base is too large')
  }
  if (b == 0 || b == -1 || b == 1) HALT('BigNumber toBaseNbs cannot have base ' + base)
    ...
    ds = map(this.toString(b).split(''), function (b) {
      return BigNumberBaseChrs.indexOf(b)
    })
  else {
    var r,
      bn = new BigNumber(this)
    while (!bn.isZero()) {
      r = bn.mod(b)
      bn = bn.divToInt(b)
